...
(async () => {
  ...
  for (let pg = 1; pg <= 10; pg++) {
    await page.goto(
      `https://www.economist.com/finance-and-economics?page=${pg}`
    );
  ...
  browser.close();
  await fse.outputFile(moment().format('YYYY-MM') + '/' + moment().format('YYYY-MM-DD') + '/finance-and-economics.txt', data.join('\n\n'));
})();
